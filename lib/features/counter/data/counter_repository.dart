...
class CounterRepository extends MainRepository {
  ...
  Future<void> saveCount(String key, int count) =>
      box.put(key, count);

  Future<void> markZekrAsRead(String zekrKey) async {
    if (!isZekrRead(zekrKey)) await box.put(zekrKey, true);
  }

  Future<void> markAllCompleted(String completedKey) async {
    if (!isAllCompleted(completedKey)) await box.put(completedKey, true);
  }
}